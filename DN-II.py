...
class UNET(nn.Module):
    def __init__(
        self, in_channels=4, out_channels=1, features=[64,128,256],
    ):
...
class DNII(nn.Module):
    def __init__(self,features=[64,128,256],num_blocks=1):
        ...
        for idx in range(self.num_blocks):
            self.blocks.append(UNET(features=self.features))
...
#         out_old=out
        for idx in range(self.num_blocks):
            out=out+0.5*self.blocks[idx](out,x)+0.5*self.convDiff(out)
            out=self.sig(out)
            out=CubicIter(out)
